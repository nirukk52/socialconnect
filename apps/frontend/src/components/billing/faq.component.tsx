...
const useFaqList = () => {
  const { appName, appNameEnterprise, isGeneral } = useVariables();
  const user = useUser();
  const t = useT();
  return [
    ...(user?.allowTrial
      ? [
          {
            ...
            description: t(
              ...
              `To confirm credit card information ${appName} will hold $2 and release it immediately, you can cancel your subscription anytime from settings without talking to a person`
            ),
          },
        ]
      : []),
    {
      title: t(
        ...
        `Can I trust ${isGeneral ? appName : appNameEnterprise}?`
      ),
      description: t(
        'faq_postiz_gitroom_is_proudly_open_source',
        `${isGeneral ? appName : appNameEnterprise} is proudly open-source! We believe in an ethical and transparent culture, meaning that ${isGeneral ? appName : appNameEnterprise} will live forever. You can check out the entire code or use it for personal projects. To view the open-source repository, <a href="https://github.com/gitroomhq/postiz-app" target="_blank" style="text-decoration: underline;">click here</a>.`
      ),
    },
    {
      ...
      description: t(
        ...
        `${isGeneral ? appName : appNameEnterprise} allows you to schedule your posts between different channels.
