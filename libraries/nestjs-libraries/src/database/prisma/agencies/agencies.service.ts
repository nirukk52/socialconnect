...
const getAppUrl = () =>
  process.env.APP_URL || process.env.FRONTEND_URL || 'https://socialconnect.app';
  ...
  async approveOrDecline(email: string, action: string, id: string) {
    ...
    if (action === 'approve') {
      await this._notificationService.sendEmail(
        agency?.user?.email!,
        `Your Agency has been approved and added to ${getAppName()} 🚀`,
        `
<html lang="en">
...
<head>
    ...
    <title>Your Agency has been approved and added to ${getAppName()} 🚀</title>
</head>

<body style="font-family: Arial, sans-serif; margin: 0; padding: 0;">
  Hi there, <br /><br />
  Your agency ${agency?.name} has been added to ${getAppName()}!<br />
  You can <a href="${getAppUrl()}/agencies/${agency?.slug}">check it here</a><br />
  It will appear on the main agency of ${getAppName()} in the next 24 hours.<br /><br />
</body>
</html>`
      );
...
<body style="font-family: Arial, sans-serif; margin: 0; padding: 0;">
  Hi there, <br /><br />
  Your agency ${agency?.name} has been declined to ${getAppName()}!<br />
  ...
  async createAgency(user: User, body: CreateAgencyDto) {
    ...
    await this._notificationService.sendEmail(
      process.env.APP_AGENCY_EMAIL || 'support@socialconnect.app',
      'New agency created',
      `
<html lang="en">
